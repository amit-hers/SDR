...
cat > "$W/status" <<'EOF'
#!/bin/sh
printf '%s\n' '{"unit":{"serial":"TEST"},"peer":{"compatibility":"COMPATIBLE"}}'
EOF
chmod 700 "$W/status"
...
echo "$bundle" | grep -q '"bundle_version":1'
...
echo "diagnostics API authentication/read-only/negative paths: PASS"
