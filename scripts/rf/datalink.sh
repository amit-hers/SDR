...
echo "$PW" | sudo -S bash -c \
  "cd $REPO && ${SDR_TX_BUF:+SDR_TX_BUF=$SDR_TX_BUF} ${SDR_TXDUMP:+SDR_TXDUMP=$SDR_TXDUMP} ${SDR_TX_SCALE:+SDR_TX_SCALE=$SDR_TX_SCALE} ${BURST:+SDR_BURST_LOG=/tmp/burst_A.log} SDR_PROFILE=1 nohup $BIN --config config.json > /tmp/dA.log 2>&1 &" 
sleep 3
echo "$PW" | sudo -S bash -c \
