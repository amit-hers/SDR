...
// significant, no shift -- so full scale is +-32767.
//
...
// 1.533x its nominal, so 2047 put the peak at 3138 -- 9.6% of the available
// range, leaving 20.4 dB of digital headroom unused.
//
// Swept at 2 MHz QPSK, 60 s per point (SDR_TX_SCALE):
//   scale   dBfs   recovered   CRC fail
//    2047  -24.1       95.1%      4.30%
//    4096  -18.1       90.1%      0.02%
//    8192  -12.0       98.9%      0.02%
//   16384   -6.0       99.0%      0.00%
// 8192 rather than 16384: the last doubling buys nothing measurable, and this
// leaves the peak at 12530, 8 dB below full scale -- headroom that a higher
// peak-to-average modulation will want.
...
// Latency deadline for flushing a partly-filled staging buffer, so a quiet
// link still delivers its last frames. Matches the daemon's own value.
static constexpr int TX_LATENCY_MS = 8;

// Scale and clamp one component to the DAC word. The clamp cannot fire at the
// default scale; it exists so a later amplitude, rolloff or aggregate change
// cannot silently wrap the int16.
...
SoftwarePhy::SoftwarePhy(const Params& p, PlutoSDR& radio, StageProfiler& prof,
                         LinkStats& link)
    : cfg_(p), radio_(radio), prof_(prof), stats_(link) {
    if (cfg_.fec)     fec_ = std::make_unique<ReedSolomon>();
    if (cfg_.encrypt && cfg_.aes_key) aes_ = std::make_unique<AESCipher>(cfg_.aes_key);

    tx_mod_ = cfg_.tx_mod;
    if (!isWireMod(tx_mod_)) tx_mod_ = ModCode::QPSK;
    ...
    tx_capacity_    = radio_.txCapacity();
    sample_rate_tx_ = static_cast<double>(cfg_.bw_mhz) * 1e6 * cfg_.samples_per_symbol;
    interp_         = std::make_unique<RRCInterp>(cfg_.samples_per_symbol);
    stage_.reserve(tx_capacity_ * 2);
    ...
    std::cerr << "[sdr] phy: software (host runs the PHY; IQ crosses the link)\n"
              << "[sdr] TX scale " << tx_scale_ << " of " << TX_FULL_SCALE
              << " (" << (20.0 * std::log10(tx_scale_ / TX_FULL_SCALE))
              << " dB below full scale; peak lands near "
              << (tx_scale_ * TX_PEAK_OVERSHOOT) << ")\n";
}

SoftwarePhy::~SoftwarePhy() { stop(); }

bool SoftwarePhy::configure(const IFramePhy::Config& c) {
    // Only the fields a running link may change are applied here; the rest
    // come from the daemon Config at construction. Applied one at a time
    // because the radio has no atomic multi-set, so a failure part-way leaves
    // the earlier fields applied -- the caller is told and re-reads state.
    if (c.tx_freq_hz) radio_.setTxFrequency(static_cast<long long>(c.tx_freq_hz));
    if (c.rx_freq_hz) radio_.setRxFrequency(static_cast<long long>(c.rx_freq_hz));
    radio_.setTxAttenuation(static_cast<double>(c.tx_atten_db));
    if (isWireMod(c.tx_mod)) tx_mod_ = c.tx_mod;
    return true;
}

void SoftwarePhy::applyRealtime(const char* who, int core) {
    if (cfg_.rt_priority > 0) {
        sched_param sp{};
        sp.sched_priority = cfg_.rt_priority;
        if (pthread_setschedparam(pthread_self(), SCHED_FIFO, &sp) != 0)
            std::cerr << "[sdr] " << who << ": SCHED_FIFO failed; normal priority\n";
    }
    if (cfg_.pin_cores && core >= 0) {
        cpu_set_t set; CPU_ZERO(&set); CPU_SET(static_cast<unsigned>(core), &set);
        pthread_setaffinity_np(pthread_self(), sizeof(set), &set);
    }
}

bool SoftwarePhy::start() {
    if (running_.exchange(true)) return true;
    pusher_thread_  = std::thread(&SoftwarePhy::txPusherThread, this);
    capture_thread_ = std::thread(&SoftwarePhy::captureThread,  this);
    rx_thread_      = std::thread(&SoftwarePhy::rxThread,       this);
    return true;
}

void SoftwarePhy::stop() {
    if (!running_.exchange(false)) return;
    // Wake every waiter BEFORE joining anything: a predicate is only
    // re-tested on notify, so notifying after joining deadlocks shutdown.
    sq_cv_.notify_all();
    ...
    // Anything never transmitted is accounted for, exactly once -- stop() is
    // reachable twice (explicitly and from the destructor), and counting the
    // residue twice made the invariant read VIOLATED by different amounts in
    // each of the two reports.
    if (!accounted_.exchange(true)) {
        std::lock_guard<std::mutex> lk(sq_mu_);
        uint64_t left = 0, frames = 0;
        for (const auto& b : sample_q_) left += b.size() / 2;
        for (auto f : sample_frames_)   frames += f;
        {
            std::lock_guard<std::mutex> sl(stage_mu_);
            left   += stage_.size() / 2;
            frames += staged_frames_;
        }
        if (left) {
            dropped_samples_.fetch_add(left, std::memory_order_relaxed);
            drop_shutdown_  .fetch_add(left, std::memory_order_relaxed);
            frames_dropped_ .fetch_add(frames, std::memory_order_relaxed);
        }
    }
}

bool SoftwarePhy::flushStage(bool force) {
    // Caller holds stage_mu_.
    if (stage_.empty()) return true;
    std::unique_lock<std::mutex> lk(sq_mu_);
    if (sample_q_.size() >= SAMPLE_Q_MAX) {
        if (!force) { sq_stalls_.fetch_add(1, std::memory_order_relaxed); return false; }
        // Real backpressure: wait for room rather than giving up. Giving up is
        // what discarded samples -- the buffer then grew past txCapacity and
        // txPush clamped, losing ~3300 frames a run at 4 MHz. Blocking pushes
        // the stall up to the TAP reader, which drops its OLDEST packet and
        // counts it: loss at the ingress, where it is visible.
        sq_stalls_.fetch_add(1, std::memory_order_relaxed);
        const auto t0 = std::chrono::steady_clock::now();
        while (sample_q_.size() >= SAMPLE_Q_MAX && running_.load())
            sq_cv_.wait_for(lk, std::chrono::milliseconds(20), [&] {
                return sample_q_.size() < SAMPLE_Q_MAX || !running_.load(); });
        backpressure_us_.fetch_add(static_cast<uint64_t>(
            std::chrono::duration_cast<std::chrono::microseconds>(
                std::chrono::steady_clock::now() - t0).count()), std::memory_order_relaxed);
        backpressure_n_.fetch_add(1, std::memory_order_relaxed);
        if (sample_q_.size() >= SAMPLE_Q_MAX) return false;   // stopping
    }

    TxBurstMeta meta;
    meta.seqs   = std::move(staged_seqs_);
    meta.offs   = std::move(staged_offs_);
    meta.forced = staged_forced_;
    if (burst_log_.is_open()) {
        double acc = 0.0, si = 0.0, sq = 0.0; int pk = 0; uint32_t clip = 0;
        for (size_t i = 0; i + 1 < stage_.size(); i += 2) {
            const int vi = stage_[i], vq = stage_[i + 1];
        ...
        const size_t np = stage_.size() / 2;
        meta.peak = pk; meta.rms = std::sqrt(acc / double(np));
        meta.clipped = clip; meta.dc_i = si / double(np); meta.dc_q = sq / double(np);
    }
    queued_samples_.fetch_add(stage_.size() / 2, std::memory_order_relaxed);
    sample_q_.emplace_back(std::move(stage_));
    sample_frames_.push_back(staged_frames_);
    sample_meta_.push_back(std::move(meta));
    lk.unlock();
    sq_cv_.notify_one();

    stage_ = std::vector<int16_t>();
    stage_.reserve(tx_capacity_ * 2);
    staged_frames_ = 0;
    staged_seqs_ = {}; staged_offs_ = {};
    staged_forced_ = false;
    return true;
}

void SoftwarePhy::flushPending() {
    // Called when the producer has nothing more to offer right now. Sends a
    // partly-filled buffer only once it has waited out the latency deadline,
    // so a quiet link still delivers its last frames promptly while a busy
    // one keeps batching.
    std::lock_guard<std::mutex> sl(stage_mu_);
    if (stage_.empty()) return;
    if (std::chrono::steady_clock::now() - stage_started_ <
        std::chrono::milliseconds(TX_LATENCY_MS)) return;
...
bool SoftwarePhy::sendFrame(const uint8_t* payload, size_t len,
    ...
    // goes on the wire -- preamble, sync word, header, CRC, FEC, encryption --
    // is the radio's business. That is what lets a DTC radio take this over
    // without the daemon changing.
    Framer framer;
    ...
    { StageProfiler::Scope sc(prof_, StageProfiler::TX_CONV, iq_shaped_.size());
      iq_hw_.resize(iq_shaped_.size() * 2);
      for (size_t i = 0; i < iq_shaped_.size(); ++i) {
          iq_hw_[i * 2]     = toDac(iq_shaped_[i].real(), tx_scale_, clamped_);
          iq_hw_[i * 2 + 1] = toDac(iq_shaped_[i].imag(), tx_scale_, clamped_);
      } }

    std::lock_guard<std::mutex> sl(stage_mu_);
    gen_samples_.fetch_add(iq_shaped_.size(), std::memory_order_relaxed);
    frames_offered_.fetch_add(1, std::memory_order_relaxed);

    // Make room BEFORE appending, and refuse if room cannot be made.
    // Appending regardless is what grew the buffer past txCapacity and let
    // txPush throw the tail away without a word.
    if (stage_.size() / 2 + iq_shaped_.size() > tx_capacity_) {
        stage_forces_.fetch_add(1, std::memory_order_relaxed);
        staged_forced_ = true;
        if (!flushStage(/*force=*/true)) {
            dropped_samples_ .fetch_add(iq_shaped_.size(), std::memory_order_relaxed);
            drop_backpressure_.fetch_add(iq_shaped_.size(), std::memory_order_relaxed);
            frames_dropped_  .fetch_add(1, std::memory_order_relaxed);
            return false;
        }
    }
    if (iq_shaped_.size() > tx_capacity_) {
        // Larger than a whole radio buffer: pushed on its own, and txPush
        // clamps, so account for what it actually took.
        int pushed = radio_.txPush(iq_hw_.data(), iq_shaped_.size());
        const size_t took = (pushed > 0) ? static_cast<size_t>(pushed) : 0;
        pushed_samples_.fetch_add(took, std::memory_order_relaxed);
        if (took < iq_shaped_.size()) {
            const size_t lost = iq_shaped_.size() - took;
            dropped_samples_.fetch_add(lost, std::memory_order_relaxed);
            drop_oversize_  .fetch_add(lost, std::memory_order_relaxed);
            frames_dropped_ .fetch_add(1, std::memory_order_relaxed);
        } else {
            frames_on_air_.fetch_add(1, std::memory_order_relaxed);
        }
        return true;
    }
    ...
    ++staged_frames_;
    if (burst_log_.is_open()) {
        staged_seqs_.push_back(seq);
        staged_offs_.push_back(static_cast<uint32_t>(stage_.size() / 2));
    }
    if (stage_.empty()) stage_started_ = std::chrono::steady_clock::now();
    stage_.insert(stage_.end(), iq_hw_.begin(), iq_hw_.end());
    return true;
}

void SoftwarePhy::txPusherThread() {
    ...
    while (running_.load()) {
        ...
        {
            std::unique_lock<std::mutex> lk(sq_mu_);
            sq_cv_.wait_for(lk, std::chrono::milliseconds(5), [&] {
                return !sample_q_.empty() || !running_.load(); });
            // An empty sample queue does NOT mean the offered traffic has
            // stopped -- it drains every few milliseconds under load. Flushing
            // on it batched 3.66 frames per burst instead of 6.4. The producer
            // calls flushPending() when it actually has nothing more.
            if (sample_q_.empty()) continue;
            buf    = std::move(sample_q_.front());   sample_q_.pop_front();
            frames = sample_frames_.front();         sample_frames_.pop_front();
            meta   = std::move(sample_meta_.front()); sample_meta_.pop_front();
        }
        sq_cv_.notify_one();
        ...
        if (now < next_tx_allowed) {
            ...
            duty_waits_us_.fetch_add(duty_wait_us, std::memory_order_relaxed);
        ...
        const double air = static_cast<double>(want) / sample_rate_tx_;
        air_seconds_.store(air_seconds_.load(std::memory_order_relaxed) + air,
                           std::memory_order_relaxed);
        // Pace against when the samples finish being RADIATED, not when
        // txPush returned -- that call hands off to DMA and returns long
        // before the air is busy, and pacing against it let measured duty hit
        // 125% of a configured 65%.
        ...
        const bool ok = (pushed >= 0 && static_cast<size_t>(pushed) == want);
        const size_t took = (pushed > 0) ? static_cast<size_t>(pushed) : 0;
        pushed_samples_.fetch_add(took, std::memory_order_relaxed);
        if (took < want) {
            const size_t lost = want - took;
            dropped_samples_.fetch_add(lost, std::memory_order_relaxed);
            drop_short_push_.fetch_add(lost, std::memory_order_relaxed);
            frames_dropped_ .fetch_add(frames, std::memory_order_relaxed);
        }
        if (ok) { pushes_ok_.fetch_add(1, std::memory_order_relaxed);
                  frames_on_air_.fetch_add(frames, std::memory_order_relaxed); }
        else      pushes_short_.fetch_add(1, std::memory_order_relaxed);

        if (burst_log_.is_open()) {
            const uint64_t id = burst_id_.fetch_add(1, std::memory_order_relaxed);
            std::lock_guard<std::mutex> lg(burst_log_mu_);
            burst_log_ << "[TXB] burst=" << id << " frames=" << frames
                       << " samples=" << want
                       << " air_us=" << static_cast<uint64_t>(air * 1e6)
                       << " push=" << (ok ? "ok" : "short")
                       << " forced=" << (meta.forced ? 1 : 0)
                       << " peak=" << meta.peak << " rms=" << meta.rms
                       << " crest=" << (meta.rms > 0 ? meta.peak / meta.rms : 0.0)
                       << " clipped=" << meta.clipped
                       << " dc_i=" << meta.dc_i << " dc_q=" << meta.dc_q
...
void SoftwarePhy::captureThread() {
    ...
    auto deliver = [&](const SplitModem::Result& sm, float rssi, float snr,
        ...
        bool this_frame_ok = false;
            ...
            for (uint8_t byte : sm.bytes) {
                ...
                // Everything below the boundary is finished: a frame has
                // been recovered from the air. What happens to it next --
                // ARQ, TAP delivery, routing, link adaptation -- is the
                // daemon's business, so hand it over and stop here.
                //
                // This callback is exactly what a DTC radio puts on the wire.
                // Nothing past this point depends on the sample rate, which
                // is why moving the line here is what makes Ethernet traffic
                // scale with payload instead of with IQ.
...
std::string SoftwarePhy::rxReport(double wall) const {
    std::ostringstream o;
    (void)wall;

        {
            ...
            o << "RX-TRANSPORT pulls=" << rx_pulls_.load()
        ...
        o << "carrier sense: defers=" << cs_defers_.load()
        ...
        // where offered load is being shed.
        {
            ...
            if (wr_fails_.load()) {
                ...
                o << "RX-RECOVER failed_decode_advances=" << f
                ...
                if (wr_hdr_adv_n_.load())
                    o << "    header-derived frame end was AHEAD of the resume point in "
                ...
                if (pn) {
                    ...
                    if (found)
                        o << "    mean distance from resume to the next real preamble="
            ...
            }
            {
                ...
                o << "RX-CARRY windows_clipped_at_buffer_edge=" << tw
                          << " deferred=" << ce
            ...
            if (ps_n_[PS_FIRST].load() || ps_n_[PS_AFTER_OK].load()) {
                static const char* kCls[] = {"first     ", "after_ok  ", "after_fail"};
                o << "RX-PSYNC  (MIN_QUALITY=" << PreambleSync::MIN_QUALITY
                ...
                if (pn) {
                    o << "RX-PSYNC probe (wide rescan of failures, n=" << pn << ")"
            ...
            if (bd_windows_.load()) {
                ...
                o << "RX-BURST seq adjacency across windows: pairs=" << sp
                          << " adjacent=" << sc
            ...
            o << "RX-WALK exits: no_sync=" << walk_exit_nosync_.load()
        ...
        }
        {
                      ...
                      << " weak_syms/frame=" << avg(t_fail_weak_.load(), tf) << "\n";
            {
                ...
                auto a=[&](const std::atomic<uint64_t>& x){ return cn ? double(x.load())/double(cn)/1000.0 : 0.0; };
                o << "RX-COSTAS n=" << cn
        ...
        }
        {
            ...
            o << "TX-BATCH pushes=" << tx.pushes
                      << " samples/push=" << per
                      << " (" << (radio_.txCapacity() ? 100.0*per/double(radio_.txCapacity()) : 0.0)
                      << "% of buffer)"
                      << "\n";
        }
        o << "TX-BLOCK duty_defers=" << tx_duty_defers_.load()
        ...
        o << "alignment retry: attempts=" << align_attempts_.load()
...
IFramePhy::Stats SoftwarePhy::stats() const {
    Stats s;
    s.frames_offered = frames_offered_.load();
    s.frames_on_air  = frames_on_air_.load();
    s.frames_dropped = frames_dropped_.load();
    // The software PHY streams IQ, so what crosses the link is samples, not
    // frames. Reporting it here is what makes the DTC comparison measurable
    // rather than asserted.
    s.host_link_tx_bytes = pushed_samples_.load() * 4;
    s.host_link_rx_bytes = rx_pulled_samples_.load() * 4;
    s.air_seconds        = air_seconds_.load();
    s.frames_decoded     = stats_.frames_rx_good.load();
    s.frames_crc_failed  = stats_.frames_rx_bad.load();
    s.bursts_detected    = stats_.bursts_detected.load();
    s.temp_c = radio_.getTemp();
    return s;
}

std::string SoftwarePhy::txReport(double wall_s) const {
    const uint64_t gen = gen_samples_.load(), q = queued_samples_.load();
    const uint64_t pu  = pushed_samples_.load(), dr = dropped_samples_.load();
    std::ostringstream o;
    o << "TX-SAMPLES generated=" << gen << " queued=" << q
      << " pushed=" << pu << " dropped=" << dr
      << " (" << (gen ? 100.0 * double(dr) / double(gen) : 0.0) << "%)\n"
      << "    drops by cause: backpressure=" << drop_backpressure_.load()
      << " short_push=" << drop_short_push_.load()
      << " oversize=" << drop_oversize_.load()
      << " at_shutdown=" << drop_shutdown_.load()
      << " frames_dropped=" << frames_dropped_.load() << "\n";
    const int64_t gap = int64_t(gen) - int64_t(pu) - int64_t(dr);
    o << "    invariant generated == pushed + dropped : "
      << (gap == 0 ? "HOLDS" : "VIOLATED");
    if (gap != 0) o << " by " << gap << " samples";
    o << "\n";
    const uint64_t bn = backpressure_n_.load();
    o << "    backpressure waits=" << bn
      << " total=" << (backpressure_us_.load() / 1000) << " ms"
      << " mean=" << (bn ? double(backpressure_us_.load()) / double(bn) / 1000.0 : 0.0)
      << " ms  (latency is the intended cost; samples are not)\n"
      << "TX-CLIP samples_clamped_at_full_scale=" << clamped_.load() << "\n"
      << "tx airtime=" << air_seconds_.load() << " s over " << wall_s << " s wall = "
      << (wall_s > 0 ? 100.0 * air_seconds_.load() / wall_s : 0.0)
      << "% duty (limit " << cfg_.tx_duty_max * 100.0 << "%)\n"
      << "tx pushes ok=" << pushes_ok_.load() << " short=" << pushes_short_.load()
      << " stage_forces=" << stage_forces_.load()
      << " sample_q_stalls=" << sq_stalls_.load() << "\n";
    return o.str();
}
